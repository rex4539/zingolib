...
impl LightClient {
    ...
    // todo use helpers
    pub async fn do_addresses(&self, subset: UASubset) -> JsonValue {
        ...
        for address in self.wallet.wallet_capability().addresses().iter() {
            let local_address = match subset {
                UASubset::Orchard => zcash_keys::address::UnifiedAddress::from_receivers(
                    address.orchard().copied(),
                    None,
                    None,
                )
                .expect("To create a new address."),
                UASubset::Shielded => zcash_keys::address::UnifiedAddress::from_receivers(
                    address.orchard().copied(),
                    address.sapling().copied(),
                    None,
                )
                .expect("To create a new address."),
                UASubset::All => address.clone(),
            };
